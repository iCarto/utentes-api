...
def exploracao_documentacao(request):
    ...
    if path_info['exploracao_id'] is not None:
        return {
            ...
            'type': 'folder',
            'url': request.route_url('api_exploracao_documentacao', subpath=format_subpath(path_info)),
            'zip_url': request.route_url('api_exploracao_documentacao_zip', subpath=format_subpath(path_info)),
            'files': request.route_url('api_exploracao_documentacao_files', subpath=format_subpath(path_info)),
            'path': request.route_url('api_exploracao_documentacao_path', subpath=format_subpath(path_info)),
            'permissions': get_folder_permissions(request, path_info['departamento'], path_info['unidade'])
        }
    else:
...
def exploracao_documentacao_path(request):
    ...
    for folder_level, folder_name in enumerate(subpath):
        ...
        if folder_level == 0:
            folder_name = request.db.query(Exploracao.exp_id).filter(Exploracao.gid == folder_name).one()[0]
        path.append(get_folder_path(request, folder_id, folder_name, folder_level + 1, subpath))
    return path
...
def get_folder_path(request, folder_id, folder_name, folder_level, subpath):
    return {
        ...
        'name': folder_name,
        'type': 'folder',
        'url': request.route_url('api_exploracao_documentacao', subpath=subpath[0:folder_level]),
        'files': request.route_url('api_exploracao_documentacao_files', subpath=subpath[0:folder_level]),
        'path': request.route_url('api_exploracao_documentacao_path', subpath=subpath[0:folder_level])
    }
    ...
    if path_info['departamento'] is None:
        ...
        if path_info['unidade'] is None:
            ...
            if departamento != ROL_UNIDAD_DELEGACION:
                return exploracao_get_departamento_files(request, path_info['exploracao_id'], path_info['departamento'], path_info['unidade'])
            else:
                return exploracao_get_unidade_delegacion_folders(request, path_info['exploracao_id'], departamento)
        else:
            return exploracao_get_unidade_files(request, path_info['exploracao_id'], path_info['departamento'], path_info['unidade'])
...
def init_departamentos_json_array(request, exploracao_id):
    ...
    for departamento in departamentos:
        departamentos_json_array.append({
            ...
            'type': 'folder',
            'name': departamento,
            'url': request.route_url('api_exploracao_documentacao', subpath=[exploracao_id, departamento]),
            'path': request.route_url('api_exploracao_documentacao_path', subpath=[exploracao_id, departamento]),
            'files': request.route_url('api_exploracao_documentacao_files', subpath=[exploracao_id, departamento])
        })
    return departamentos_json_array
...
def exploracao_get_unidade_delegacion_folders(request, exploracao_id, departamento):
    ...
    res = request.db.query(
            ...
            label('num_files', func.count(Documento.gid)),
            label('last_created_at', func.max(Documento.created_at))
        ).filter(Documento.exploracao == exploracao_id, Documento.departamento == departamento).group_by(Documento.unidade).order_by(Documento.unidade).all()
    for row in res:
        unidade = next((unidade_json for unidade_json in unidades_json_array if unidade_json['name'] == row[0]), None)
        if unidade:
            unidade['size'] = row[1]
            unidade['date'] = row[2]
...
def init_unidades_json_array(request, exploracao_id):
    ...
    for unidade in unidades:
        unidades_json_array.append({
            ...
            'type': 'folder',
            'name': unidade.key,
            'url': request.route_url('api_exploracao_documentacao', subpath=[exploracao_id, ROL_UNIDAD_DELEGACION, unidade.key]),
            'path': request.route_url('api_exploracao_documentacao_path', subpath=[exploracao_id, ROL_UNIDAD_DELEGACION, unidade.key]),
            'files': request.route_url('api_exploracao_documentacao_files', subpath=[exploracao_id, ROL_UNIDAD_DELEGACION, unidade.key])
        })
    return unidades_json_array
...
def exploracao_get_unidade_files(request, exploracao_id, departamento, unidade):
    files = request.db.query(Documento).filter(Documento.exploracao == exploracao_id, Documento.departamento == departamento, Documento.unidade == unidade).order_by(Documento.name).all()
    jsonFiles = []
    for file in files:
        jsonFile = file.__json__(request)
        jsonFile['permissions'] = get_file_permissions(request, departamento, unidade)
...
def documento_file_upload(request, path_info):
    ...
    if request.user.usergroup != ROL_ADMIN:
        if request.user.usergroup != path_info['departamento'] or (path_info['unidade'] is not None and request.user.unidade != path_info['unidade']):       
            raise badrequest_exception({
    ...
    if path_info['unidade'] is not None:
        previous_documento = previous_documento.filter(Documento.unidade == documento.unidade)
...
def exploracao_documentos_zip(request):
    ...
    if departamento is not None:
        if unidade is not None:
            query = request.db.query(Documento).filter(Documento.exploracao == exploracao_id, Documento.departamento == departamento, Documento.unidade == unidade)
        else:
            ...
            for documento in documentos:
                ...
                if os.path.isfile(path_in_disk):
                    ...
                    if departamento is None:
                        path_in_zip = os.path.join(documento.departamento, documento.unidade if documento.unidade is not None else '', name)
                    else:
                        if unidade is None:
                            path_in_zip = os.path.join(documento.unidade if documento.unidade is not None else '', name)
                        else:
...
def find_documento(request, name, path_info):
    query = request.db.query(Documento).filter(Documento.exploracao == path_info['exploracao_id'], Documento.departamento == path_info['departamento'], Documento.name == name)
    if path_info['unidade'] is not None:

...
def dump(session):
    ...
    args_str = '-U {username} -h {host} -w -p {port} -d {database} -Z9 -Fc -O -x -E UTF-8 --schema=utentes -f'.format(**params)
    args = [pg_dump]
    ...
    if exitcode != 0:
        raise DBUtilsException(u'Não foi possível exportar o banco de dados', err)
    return filename

...
def restore(session, dumpfile):
    ...
    if not is_same_version(session, dumpfile):
        raise DBUtilsException('A versão da cópia é diferente da do banco de dados e não é possível restaurar')
    exitcode, err = terminate_database_connections(session)
    if exitcode != 0:
        raise DBUtilsException('Não foi possível ler fechar a conexão com o banco de dados para fazer um backup', err)

    # TODO:
    # Cachear, memoization de los paths

    exitcode, err = clone_database(session, version() + '_auto_tmp_' + params['database'], drop_if_exists=True)
    if exitcode != 0:
        raise DBUtilsException('Não foi possível salvar o banco de dados atual com êxito', err)
    ...
    args = [pg_restore]
    args_str = '-U {username} -h {host} -p {port} -d {database} --clean --disable-triggers --single-transaction --exit-on-error --schema=utentes'.format(**params)
...
def ensure_correct_filename(dumpfile, params):
    ...
    if not match_obj:
        raise DBUtilsException('O nome do arquivo de entrada não tem o formato esperado')
    date = match_obj.group(1)
    if date_of_dumpfile(date) > datetime.date.today():
        raise DBUtilsException('Não é possível restaurar arquivos cujo nome tenha uma data após o atual')
    db = match_obj.group(2)
    if params['database'] != db:
        raise DBUtilsException('Não é possível restaurar arquivos cujo nome não corresponda ao banco de dados que está sendo usado')

...
def ensure_connection_is_available(params, env):
    ...
    if exitcode != 0:
        raise DBUtilsException('Não é possível estabelecer uma conexão com o banco de dados', err)
...
def clone_database(session, dbname, drop_if_exists=False):
    ...
    args = [createdb]
    args_str = '-U {username} -h {host} -p {port} -T {database} {dbname} '.format(dbname=dbname, **params)
...
def drop_database(session, dbname):
    ...
    args = [dropdb]
    args_str = '-U {username} -h {host} -p {port} {dbname} '.format(dbname=dbname, **params)
    args.extend(args_str.split())
...
def execute_query_quitely(query, params, env=None):
    psql = find_psql_path()
    args_str = '-U {username} -h {host} -w -p {port} -d {database} -t -q -X'.format(**params)
    args = [psql]
    args.extend(args_str.split())
    args.append('-c')
    args.append(query)
...
def _get_session(dbname='arasul'):
    ...
    engine = create_engine('postgresql://postgres:postgres@localhost:9001/{}'.format(dbname))

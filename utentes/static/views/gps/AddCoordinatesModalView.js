...
// 663568,082135983 8566051,91820535
    ...
    set_listeners: function(){
        ...
        this.org_x_widget.addEventListener('input', this.convert_coordinates.bind(this));
        this.org_y_widget.addEventListener('input', this.convert_coordinates.bind(this));
        this.org_srs_widget.addEventListener('change', this.convert_coordinates.bind(this));
        this.dest_srs_widget.addEventListener('change', this.convert_coordinates.bind(this));
        ...
        document.getElementById('zoom_to_point').addEventListener(
            'click', this.zoom_to_point.bind(this)
        );
        document.getElementById('create_point').addEventListener(
            'click', this.create_point.bind(this)
        );
    ...
    create_point: function(){
        ...
        var geojsonFeatureRaw = this.create_GeoJSON_from_points(this.wgs84_stored.lng, this.wgs84_stored.lat, label);
    ...
    are_valid_coordinates: function() {
        ...
        if (this.is_from_Geog()) {
            if (!this.well_formatted_geog_coordinate(x)) {
                var wrongXGeogFormat = 'A Coordenada X não ten o formato correcto. ie: "' + window.SIXHIARA.center[0] + '"';
            ...
            if (!this.well_formatted_geog_coordinate(y)) {
                var wrongYGeogFormat = 'A Coordenada Y não ten o formato correcto. ie: "' + window.SIXHIARA.center[1] + '"';
        ...
        if (this.is_from_UTM()) {
            if (!this.well_formatted_utm_coordinate(x)) {
                var wrongXUTMFormat = 'A Coordenada X não ten o formato correcto. ie: "1316614,053"';
            ...
            if (!this.well_formatted_utm_coordinate(y)) {
                var wrongYUTMFormat = 'A Coordenada Y não ten o formato correcto. ie: "8554543,482"';
    ...
    are_valid_geog_coordinates: function(point){
        if (!formatter().inRange(point[0], -180, 180)) { // lng_range
        ...
        if (!formatter().inRange(point[1], -90, 90)) { // lat_range
    ...
    are_valid_utm_coordinates: function(point) {
        ...
        if (this.get_org_srs() == '32737') {
            ...
            var northEast = [166021.4431, 1116915.0440];
            var southWest = [833978.5569, 10000000.00];
        }
        if (this.get_org_srs() == '32736') {
            // https://epsg.io/32736
            var northEast = [441867.78, 1116915.04];
            var southWest = [833978.56, 10000000.00];
        }
        if (!formatter().inRange(point[0], northEast[0], southWest[0])) { // x_range
            var wrongXRange = 'A Coordenada X não está entre ' + northEast[0] + ' e ' + southWest[0];
            this.org_x_widget.setCustomValidity(wrongXRange);
            this.display_error(wrongXRange);
            return false;
        }
        if (!formatter().inRange(point[1], northEast[1], southWest[1])) { // y_range
            var wrongYRange = 'A Coordenada Y não está entre ' + northEast[1] + ' e ' + southWest[1];
    ...
    enable_create_button: function() {
        ...
        if (!this.inside_working_area(this.wgs84_stored)) {
            var outOfBBox = 'O ponto que você acrescentou está situado fora do ámbito de actuación.';
    ...
    remove: function() {
        this.marker && this.options.map.removeLayer(this.marker);
        return Backbone.SIXHIARA.AddCoordinatesModalView.__super__.remove.apply(this, arguments);
    },
});
